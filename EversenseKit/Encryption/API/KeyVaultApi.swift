...
class KeyVaultApi {
    ...
    static func getFleetSecret(accessToken: String) async -> SecureKeyResponse? {
        ...
        do {
            ...
            guard let response = response as? HTTPURLResponse, response.statusCode == 200 else {
                logger.error("Got invalid response from KeyVault: \((response as? HTTPURLResponse)?.statusCode ?? -1) \(String(data: data, encoding: .utf8) ?? "No data")")
                return nil
            }
    ...
    static func getFleetSecretV2(accessToken: String, serialNumber: String, nonce: String, flags: Bool, uniqueId: String) async -> SecureKeyResponse? {
        guard let url = URL(string: "\(baseUrl)/GetTxCertificate?txSerialNumber=\(serialNumber)&clientNo=\(clientNo)&nonce=\(nonce)&tx_flags=\(flags)&client_type=\(clientType)&kp_client_unique_id=\(uniqueId)") else {
            logger.error("Could not create URL...")
            return nil
        }
